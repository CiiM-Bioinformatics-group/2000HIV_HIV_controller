...
pheno3 <- pheno3 %>%
  filter(ETHNICITY != "", PC1 != "", CONTROLLER_1B != "") %>%
  mutate(
    ID_idat = ID,
...
save(OutlierResults[[2]], file = "Outlier_log.1.Rdata")
...
cor_test <- function(gene) {
  ...
  tryCatch({
    ML <- glm(as.factor(CONTROLLER_YESNO) ~ methylation + AGE+PC1+PC2, 
...
data <- readRDS("cor_info.rds")
data1 <- filter(data, CpGsite == "cg04895445")
print(data1)